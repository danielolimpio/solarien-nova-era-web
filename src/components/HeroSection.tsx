  ...
  return (
    <>
      ...
      >
        ...
        {/* Efeitos de energia - mantidos originais para desktop, reduzidos para mobile */}
        ...
        {/* Conteúdo principal - ajustado apenas para mobile */}
        <header className="relative z-10 text-center px-4 md:px-4 pt-24 md:pt-20">
          <div className="max-w-4xl mx-auto">
            ...
            {/* Texto animado com atributos SEO - mantido original para desktop */}
            <div data-seo-context="energia-renovavel-limpa" className="mb-6">
            ...
            {/* Seção de benefícios - ajustada apenas para mobile */}
            <div 
              className="flex flex-col md:flex-row justify-center items-center gap-4 md:gap-6 mb-12 hover:animate-soft-pulse transition-all duration-300" 
              itemScope 
              itemType="https://schema.org/ItemList"
              data-seo-benefits="sem-custos-obras-placas"
            >
              {benefitsData.map(({ Icon, text }, index) => (
                <div key={text} className="flex items-center gap-2" itemProp="itemListElement" itemScope itemType="https://schema.org/ListItem">
                  <Icon className="w-4 md:w-5 h-4 md:h-5 text-[#02ff91] flex-shrink-0" aria-hidden="true" />
                  <span className="text-gray-300 font-medium text-sm md:text-base" itemProp="name">{text}</span>
                  <meta itemProp="position" content={`${index + 1}`} />
                </div>
              ))}
            </div>
            
            {/* Botões - ajustados para mobile em coluna, desktop em linha */}
            <div className="flex flex-col md:flex-row gap-4 md:gap-6 justify-center items-center">
              <DiscountSimulator>
                <Button 
                  className="w-full md:w-auto bg-gradient-to-r from-[#02ff91] to-[#00c26f] hover:from-[#00c26f] hover:to-[#00844e] text-black font-bold px-8 md:px-12 py-4 md:py-6 text-lg md:text-xl rounded-xl md:rounded-2xl transition-all duration-300 transform hover:scale-105 md:hover:scale-110 shadow-2xl border-2 border-[#02ff91] hover:border-[#00c26f] glow-effect hover:animate-gentle-glow will-change-transform"
              ...
              <Button 
                variant="outline" 
                className="w-full md:w-auto border-2 md:border-3 border-[#02ff91] text-[#02ff91] hover:bg-gradient-to-r hover:from-[#02ff91] hover:to-[#00c26f] hover:text-black font-bold px-8 md:px-12 py-4 md:py-6 text-lg md:text-xl rounded-xl md:rounded-2xl transition-all duration-300 transform hover:scale-105 md:hover:scale-110 shadow-2xl hover:shadow-[#02ff91]/50 glow-effect hover:animate-gentle-glow will-change-transform"
          ...
          {/* Estatísticas - ajustadas para mobile em 2x2, desktop em linha */}
          <div 
            className="mt-16 pb-8 md:pb-0 grid grid-cols-2 md:grid-cols-4 gap-6 md:gap-8 max-w-4xl mx-auto" 
            itemScope 
          ...
          >
            {statisticsData.map(({ value, label }) => (
              <div key={label} className="text-center hover:animate-soft-pulse transition-all duration-300">
                <div className="text-2xl md:text-4xl font-bold bg-gradient-to-r from-[#02ff91] via-[#00c26f] to-[#00844e] bg-clip-text text-transparent mb-2" itemProp="description">{value}</div>
                <div className="text-gray-300 text-xs md:text-base leading-tight">{label}</div>
              </div>
            ))}
          </div>
        </header>
