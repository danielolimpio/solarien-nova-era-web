...
import { PremiumSection } from './premium/PremiumUI';
...
const SolarPlantsCarousel = () => {
  ...
  return (
    <PremiumSection label="Infraestrutura">
      <div className="text-center mb-12">
        <h2 id="usinas-heading" className="text-4xl md:text-5xl font-light text-white tracking-tight mb-6">
          Usinas Solares
        </h2>
      ...
      <div className="flex flex-col lg:flex-row gap-2 w-full" role="region" aria-label="Carrossel de usinas solares">
        {solarPlants.map((plant, index) => (
          <article
            key={plant.id}
            className={`relative overflow-hidden cursor-pointer transition-all duration-700 ease-in-out border border-white/10 ${
          ...
          >
            ...
            <div className="absolute inset-0 flex flex-col justify-end p-8">
              <div className="text-xs uppercase tracking-[0.2em] text-white/60 mb-2">{plant.capacity}</div>
              ...
              <div className={`transition-all duration-700 overflow-hidden ${
                ...
                <p className="text-white/60 font-light text-sm mb-2">{plant.location}</p>
                <p className="text-white/70 font-light leading-relaxed">{plant.description}</p>
              </div>
            </div>
          </article>
        ))}
